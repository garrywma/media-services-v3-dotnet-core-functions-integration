...
namespace advanced_vod_functions_v3
{
    ...
    public static class GetAssetBlobContainer
    {
        [FunctionName("GetAssetBlobContainer")]
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function, "post", Route = null)] HttpRequest req,
            ILogger log)
        {
            log.LogInformation($"AMS v3 Function - GetAssetBlobContainer was triggered!");
            ...
            try
            {
               ...
               return (ActionResult)new OkObjectResult(assetContainerSas.AssetContainerSasUrls[0].Split('/')[3].Split('?')[0]);
            }
            catch (ApiErrorException e)
            {
                log.LogInformation($"ERROR: AMS API call failed with error code: {e.Body.Error.Code} and message: {e.Body.Error.Message}");
